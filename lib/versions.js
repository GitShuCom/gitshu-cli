...
function listVersions() {
    ...
    .map(function(tag) {
        ...
        if (pkg.name != 'gitshu') return null;
...
function availableVersions() {
    return initNPM()
    .then(function() {
        return Q.nfcall(npm.commands.view, ["gitshu", "versions", "dist-tags"], true);
    })
...
function installVersion(version, forceInstall) {
    ...
    .spread(function(tmpDir) {
        ...
        console.log("Installing GitShu", version);
        return Q.nfcall(npmi.bind(npmi), options).thenResolve(tmpDir);
    })
    .then(function(tmpDir) {
        var gitbookRoot = path.resolve(tmpDir, "node_modules/gitshu");
        ...
        if (!tags.isValid(version)) throw "Invalid GitShu version, should satisfies "+config.GITBOOK_VERSION;
...
function uninstallVersion(version) {
    if (!version) return Q.reject(new Error("Need a version of GitShu to uninstall"));
    var outputFolder = path.resolve(config.VERSIONS_ROOT, version);
...
function getVersion(version) {
    ...
    .then(function(resolved) {
        ...
        if (!gitbook) throw "GitShu Version "+resolved.tag+" is corrupted";
        return gitbook;
    });
}
